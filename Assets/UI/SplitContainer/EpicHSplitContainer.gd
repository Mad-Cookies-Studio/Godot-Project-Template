...
func set_init_size(new : int) -> void:
	if !Engine.is_editor_hint(): return
	initial_size = new
	$SortingContainer/Section1.rect_min_size.x = new 
...
func set_grabber_size(new : int) -> void:
	if !Engine.is_editor_hint(): return
	grabber_size = new
	$SortingContainer/Grabber.rect_min_size.x = new
...
func set_grabber_area_size(new : int) -> void:
	...
	$SortingContainer/Grabber.get_child(0).rect_min_size.x = new
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseMotion:
		$SortingContainer/Section1.rect_min_size.x = get_local_mouse_position().x


func _on_GrabberArea_gui_input(event: InputEvent) -> void:
